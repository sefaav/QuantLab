...
def render_html_report(
    ...
    *,
    ...
    robustness_html = _render_robustness(robustness)
...
<main>
  ...
  <section><h2>Results</h2>
    ...
    <h3>Full-sample headline metrics</h3>
    {_table_html(metrics_table(result))}
    <h3>Full-sample gross vs net</h3>
    {_table_html(gross_net_table(result))}
    <h3>Full-sample performance by year</h3>
  ...
  <footer>Generated by QuantLab &middot; reproducible from config.yaml given the
    same code, data and dependency versions recorded in metadata.json</footer>
</main>
</body></html>"""
...
def _render_robustness(robustness: dict[str, Any] | None) -> str:
